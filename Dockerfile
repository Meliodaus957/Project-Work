...
ENTRYPOINT ["/app/wait-for-it.sh", "https://www.saucedemo.com/", "--"]

# Команда, которая будет выполнена после того, как wait-for-it.sh завершит ожидание
CMD ["pytest", "-v", "tests/test_opencart.py", "--base_url=https://www.saucedemo.com/", "--browser=chrome", "--bv=latest", "--executor=selenoid"]